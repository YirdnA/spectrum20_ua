...
\item Коли EMR проходить через елементи, певні діапазони довжин хвиль поглинаються, а деякі нові випромінюються. Це поглинання та емісія створюють характерні спектральні лінії для кожного елемента, що корисно для визначення складу далеких зірок. Ці лінії використовуються для підтвердження величини червоного зміщення далеких зірок.

\item
 Коли фотон потрапляє на атом, він може бути поглинений, якщо енергія точно відповідає.
 Енергетичний рівень електрона підвищується --- фактично утримуючи випромінювання.
 Новий фотон з певною довжиною хвилі створюється при вивільненні енергії.
 Стрибок енергії є дискретним кроком, і в атомі існує багато можливих дискретних рівнів енергії.

\item \begin{minipage}[t]{3.5in} Йоганн Бальмер створив цю формулу, що визначає довжину хвилі емісії фотона ($\lambda$); де $m$ --- початковий цілочисельний номер енергетичного рівня електрона, а $n$ --- кінцевий: \end{minipage}%
\hspace{0.2in}
...
\item Більша частина міжзоряної речовини складається з найпростішого атома --- водню. Лінії емісії та поглинання видимого спектра водню показані нижче:
		...
		\absorbemit{380.25}{385}{}{0}
{
	...
	\uput{2pt}[180](70,.17){Лінія емісії}\psline{->}(70,.17)(86,.17)(86,.12)(102,.12)
	\uput{2pt}[180](70,-.10){Лінія поглинання}\psline{->}(70,-.10)(86,-.10)(86,.04)(102,.04)
	\uput{2pt}[0](140,-.16){Назва серії Бальмера}\psline{->}(140,-.16)(127,-.16)(127,-.09)(113,-.09)
}
	}
}
\vspace{0.2in}
...
\rput(0,-.65){
	\psframebox[linestyle=none]{
		...
		\rput[b]{90}(-.05,.45){Потужність}
		\rput[t](.5,-.05){Довжина хвилі}
		\psset{border=1pt, bordercolor=Black, fillstyle=none, linearc=0,linecolor=green}
		\psline{o-}(.08,.6)(.5,.6)\uput{2pt}[0](.5,.6){Біле каління}
		\psline{o-}(.12,.43)(.7,.43)\uput{2pt}[0](.7,.43){Червоне каління}
		\psline{o-}(.18,.25)(.9,.25)\uput{2pt}[0](.9,.25){Гаряче}
...
\begin{minipage}[t]{2.4in}
	\begin{itemize}
		\item Макс Планк визначив залежність між температурою об'єкта та його профілем випромінювання; де $R_\lambda$ --- потужність випромінювання, $\lambda$ --- довжина хвилі, $T$ --- температура:
	\end{itemize}
\end{minipage}
